...
func WriteResponse(w http.ResponseWriter, bt []byte) {
	writeResponse(w, http.StatusOK, bt, mimeJSON)
}
...
func ParseParam(w http.ResponseWriter, r *http.Request, event interface{}) error {
	jsonBytes, err := ioutil.ReadAll(r.Body)
	if err != nil {
		ProcessFail(w, r, event, "parseParam ioutil.ReadAll(r.Body) fail: "+err.Error()+", body: "+string(jsonBytes), error2.ErrorCodes.ToAPIErr(error2.ErrInvalidArguments))
		return err
	}

	err = json.Unmarshal(jsonBytes, event)
	if err != nil {
		ProcessFail(w, r, event, "parseParam json.Unmarshal jsonBytes fail: "+err.Error()+", jsonBytes: "+string(jsonBytes), error2.ErrorCodes.ToAPIErr(error2.ErrInvalidArguments))
		return err
	}
...
func ProcessFail(w http.ResponseWriter, r *http.Request, requestBody interface{}, errInfo string, err error2.APIError) {
	bt, _ := json.Marshal(requestBody)
	logger.Errorf("url:%v  |  proc fail: %v  | request: %v", r.URL, errInfo, string(bt))
	WriteErrorResponseJSON(w, err, r.URL)
}
