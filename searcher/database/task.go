...
func (p *Task) GetTaskInfoByOrderId(orderId string) (ret []dict.TaskInfo, err error) {
	for i := 0; i < Count; i++ {
		if ret, err = p.getTaskInfo(bson.M{"order_id": orderId}); err == nil || err == mgo.ErrNotFound {
			return ret, nil
		}
...
func (p *Task) GetTaskInfoByName(name string) (ret []dict.TaskInfo, err error) {
	for i := 0; i < Count; i++ {
		if ret, err = p.getTaskInfo(bson.M{"name": name}); err == nil || err == mgo.ErrNotFound {
			return ret, nil
		}
		time.Sleep(time.Duration(TimeInternal) * time.Millisecond)
	}
