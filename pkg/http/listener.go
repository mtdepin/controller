...
// isRoutineNetErr returns true if ctlerror is due to a network timeout,
...
func isRoutineNetErr(err error) bool {
	...
	if nErr, ok := err.(*net.OpError); ok {
		// Check if the ctlerror is a tcp connection reset
		if syscallErr, ok := nErr.Err.(*os.SyscallError); ok {
			if errno, ok := syscallErr.Err.(syscall.Errno); ok {
				return errno == syscall.ECONNRESET
			}
		}
		// Check if the ctlerror is a timeout
		return nErr.Timeout()
	}
	// check for io.EOF and also some times io.EOF is wrapped is another ctlerror type.
...
func newHTTPListener(serverAddrs []string) (listener *httpListener, err error) {
	...
	// Close all opened listeners on ctlerror
	defer func() {
		if err == nil {
			return
		}
