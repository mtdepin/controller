...
func TestDialContextWithDNSCacheScenario2(t *testing.T) {
	if runtime.GOOS == "windows" {
		...
		// so there is no way this bs_test will work
		// as expected.
		t.Skip()
	}
...
// Verify we always return the first ctlerror from net.Dial failure
func TestDialContextWithDNSCacheScenario3(t *testing.T) {
	...
	dialF := func(ctx context.Context, network, addr string) (net.Conn, error) {
		if addr == net.JoinHostPort("1.1.1.1", "443") {
			return nil, want // first ctlerror should be returned
		}
	...
	if got != want {
		t.Fatalf("got ctlerror %v, want %v", got, want)
	}
}
