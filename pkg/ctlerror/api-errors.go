...
// APIErrorResponse - ctlerror response format
...
// APIErrorCode type of ctlerror status.
...
// ErrorCodes ctlerror code to APIError structure, these fields carry respective
// descriptions for all the ctlerror responses.
var ErrorCodes = errorCodeMap{
	...
	// Add your ctlerror structure here.
}
...
func ToAPIError(ctx context.Context, err error) APIError {
	...
	if apiErr.Code == "InternalError" {
		// If we see an internal ctlerror try to interpret
		// any underlying errors if possible depending on
		// their internal ctlerror types. This code is only
...
// GenericError - generic object layer ctlerror.
...
func (e FileSystemError) Error() string {
	return "file system ctlerror: " + e.Bucket + "/" + e.Object
}

// IncompleteBody You did not provide the number of bytes specified by the Content-Length HTTP header.
type IncompleteBody GenericError

// Error returns string an ctlerror formatted as the given text.
